...
def parseInput(input):
    ...
    for y, row in enumerate(input.split('\n')):
        for x, cell in enumerate(row):
            ...
            if cell in '#.': continue
            goals[cell] = p
    return maze, goals  # goals is a dict of cooridnates with values as keys, maze is a dict of mapvalues with coordinates as keys


def findLinks(maze, start):
    links = {}
    walk = defaultdict(lambda: [99999, {}])  # is it a list

    walk[start] = (0, set())  # walk is the distance and set of visited points?
    next = [(start, set())]  # que?

    for step in count(1):  # inifinite for with counting
        if len(next) == 0: break  # if no more in que break -> return links
        cur, next = next, []  # reasignement to empty que
        for p, ds in cur:
            for d in [1, 1j, -1, -1j]:
                c = maze[p + d]  # "look" in direction
                if c == '#' or walk[p + d][0] <= step: continue  # if there is a wall or shorter path break
                if c.islower():    # if looking at key add to links
                    links[c] = (step, ds)   # attach lookup value and ds - empty set at the beginining?
                nds = ds        # nds = ds - keys?
                if c.isupper():  # if there is a door
                    nds = nds | {c.lower()}  # add door to nds as lowercase (key)
                walk[p + d] = (step, nds)   # add counter to book of travel and set of keys
                next.append((p + d, nds))   # add to the list to check the node checked
    return links  # naturally sorted by distance get a list of paths leading to the node and keys required to get there.


def findShortest1():
    maze, goals = parseInput(input)
    # goals is a dict of cooridnates with values as keys,
    # maze is a dict of mapvalues with coordinates as keys

    allKeys = {k for k in goals if k.islower()}  # list of all keys

    links = {'@': findLinks(maze, goals['@'])}  # initialise with all the paths leading to the start
    for k in allKeys:                           # find shortest paths and keys required to get to every node
    ...
    def walk(name, needKeys):   # walk needs function # name is a name of the node?
        if len(needKeys) == 0:
            return 0

        key = name + ''.join(needKeys)    # key state at certain node
        if key in cache:
            return cache[key]
