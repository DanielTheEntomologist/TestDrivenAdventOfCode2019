    ...
    while True:
        ...
        if opcode == 99:  # halt
            ...
            try:
                ...
                # pos = pos  # + length
                unconsumed_inputs = []
                #print("waiting fo input" )
                yield (None, "waiting for input") #mem, unconsumed_inputs, pos, rel_pos, output_list, "waiting for input"
                input_list = yield
                if isinstance(input_list, list):
                    input_x = iter(input_list)
                else:
                    input_x = iter([input_list])
                #print("loaded input_x:",input_x)
            ...
            output_val = mem[pointers[0]]
            #print("yielding from comp",output_val )
            yield (output_val, "running")
            ...
            unconsumed_inputs = list(deque(input_x))
            yield (None,"error") #, mem, unconsumed_inputs, pos, rel_pos, output_list, "error"
...
# create the gen/coroutine
comp1 = stepper_computer(mem_out, inputs_out, pos_out,rel_pos_out)
#last_output = next(comp1)
...
while state not in ["halted","error"]:
    ...
    # progress to first yield statement
    #print("deciding on sending input, last output is:", last_output)
    # if last_output == "waiting for input":
        # last_output = comp1.send(input_list)
        # print( "after sending",input_list,"got",last_output)
        # print("using next")
        #last_output = next(comp1)
        # print("using next got us", last_output)
    #else:
        # print("going for next without sending")
        # last_output = next(comp1)
        # print("using next got us",last_output)
    ...
    last_output,state = next(comp1)#comp1.send(input_list)
    print("(value, state) returned:", (last_output, state))
    if state == "waiting for input":
        print("sending:", color)
        last_output = comp1.send(input_list)
        print("after sending", last_output)
        #last_output, state = next(comp1)
    #else:
        #last_output, state = next(comp1)
    time.sleep(1)

    # paint the tile
    trace[position] = last_output#[-1]

    # raport
    # print("lastOUT:", last_output)
    outputs.append(last_output)#[-1])

    # turn and move
    if last_output== 0:  # turn left
